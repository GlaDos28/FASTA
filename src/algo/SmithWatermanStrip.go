...
func SmithWatermanStrip(seqPair *SeqPair, strip Strip, wm *WeightMatrix, gapPenalty int) *AlignResult {
    ...
    dpB := make([]int, w + 1) /* Backward running line of dynamic programming pass */
    ...
    //   i   |   0    1    2    3    4    ...   N = w
    //  dpB  |   x1   x2   x3   x4   x5   ...   0  <- dpB[w] should be set to 0 before step two
    //              \    \    \    \          \
    //  dpF  |   0    x1   x2   x3   x4   ...   xN
    ...
    smithWatermanDp(startRow, endRow, firstColStart, firstColEnd, dpF, dpB, 0, 1, seqPair, wm, gapPenalty, &maxScore)
    ...
    smithWatermanDp(startRow, endRow, firstColStart, firstColEnd, dpF, dpB, 1, 1, seqPair, wm, gapPenalty, &maxScore)
    ...
    smithWatermanDp(startRow, endRow, firstColStart, firstColEnd, dpF, dpB, 1, 0, seqPair, wm, gapPenalty, &maxScore)

    // Return result

    return &AlignResult{ Score: maxScore }
}
...
func smithWatermanDp(
    ...
    for row := startRow; row <= endRow; row += 1 {
        ...
        endCol   := firstEndCol   + (row - startRow) * endColIncFactor

        for i, col := 1, startCol; col <= endCol; i, col = i + 1, col + 1 {
            dpF[i] = util.Max4(
                0,
                dpF[i - 1] + gapPenalty,                     /* left */
                dpB[i]     + gapPenalty,                     /* top */
                dpB[i - 1] + seqPair.WeightIn(wm, row, col)) /* top-left */

            *maxScore = util.MaxInt(*maxScore, dpF[i])
        }

        copy(dpB, dpF[startColIncFactor:]) /* from 0 for step one, from 1 for steps two and three */
    }
}
